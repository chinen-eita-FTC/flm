...
class BookGenre extends Model
{
    ...
    public function createBookGenre(array $array){
        $response['status'] = false;
        try{
            $target = $this->create($array);
    ...
    public function getBookGenre(int $int){
        $response['status'] = false;
        try{
            $target = $this->find($int);
            $response['status'] = true;
            return collect($response);
        } catch (Exception $e) {
            // TODO [v1.0|機能追加] 例外処理の送出方法の決定後に削除時の例外処理の追加すること 
    ...
    public function updateBookGenre(int $id,$name){
        $response['status'] = false;
        try{
            $target = $this->find($id);
